...
RESET:	      				LDI temp, Low(RAMEND)
		      				...
		      				OUTI TCCR0B,1<<WGM02|1<<CS02|0<<CS01|0<<CS00							;Set register for Fast PWM 1:256
			      			...
			      			OUTI TCCR1B,1<<WGM12|1<<WGM13|1<<CS12|0<<CS11|0<<CS10					;Set register for Fast PWM 1:256
		      				...
		      				OUTI TCCR2B,1<<WGM22|1<<CS22|0<<CS21|0<<CS20							;Set register for Fast PWM 1:256																					;need to set timers
...
INT_I2C:	      			CLI
			      			...
			      			BREQ GO_AWAY			;Go to SLA_W lable
			      			CPI temp, 0xB0			;If receive request to read while we be in data sending process
			      			BREQ STOP_W_GO_R
			      			CPI temp, 0xA8			;If receive request to our address to read(we need to send data to master)
...
LAST_BYTE:	      			LDS temp, TWDR
			      			...
			      			LDI i2ccheck, 0x02
			      			RJMP Vix
GO_AWAY:	      			LDI temp, 0b11000101
			      			RJMP Vix
STOP_W_GO_R:      			NOP
SLA_R:		      			CLR sys
			      			LDI YH, high(encoderVar)	;Load mamory address of encoderVar to Y registers
			      			LDI YL, low(encoderVar)		;Disable interrupts (global)
			      			LD temp, Y+					;Check command(high bit of mainVar)
			      			STS TWDR, temp				;Load first byte of encoderVar to TWDR
			      			LDI temp, 0b11000101
			      			STS TWCR, temp
			      			RJMP Vix
S_BYTE_R_ACK:      			inc sys
			      			LD temp, Y+
			      			STS TWDR, temp
			      			CPI sys, 6
			      			BREQ SEND_LBYTE				;Check if send last byte of wncoderVar
			      			LDI temp, 0b11000101		;Wait ACK
			      			STS TWCR, temp
			      			RJMP Vix
SEND_LBYTE:	      			LDI temp, 0b10000101		;Wait NACK
			      			STS TWCR, temp
			      			RJMP Vix
S_LBYTE_R_NACK:    			NOP
S_LBYTE_R_ACK:     			LDI temp, 0b11000101
			      			STS TWCR, temp
			      			RJMP Vix
Vix:		      			SEI
		      				RETI
...
DELAY:		      			CPI i2ccheck, 0x02
			      			BRNE DELAY					;Delay
			      			CLI							;Disable interrupts(global)
		      				LDI YH, high(mainVar+7)		;Load mamory address of mainVar to Y registers
			      			LDI YL, low(mainVar+7)		;Disable interrupts (global)
			      			LD temp, Y					;Check command(high bit of mainVar)
