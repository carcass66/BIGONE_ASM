				...
				.def left_count = R1		;Counter of left wheel
				.def right_count = R2		;Counter of right wheel
				.def temp=R16
...
;======================================== Start macro.inc==========================
				...
				.macro STSL				; STS low
				PUSH R17				; Save high register value to Stack,
				MOV R17,@1				; Move value from low register to R17,
				STS @0,R17				; Store value from R17 to variable
				POP R17
				.endm
...
RESET:			LDI temp, Low(RAMEND)
				...
				SETB DDRD,5,temp	;out PD5 - PWM_R_R (2) - output (OC0B)
				...
				SETB DDRB,1,temp	;out PB1 - PWM_R_L (1) - output (OC1A)
      			SETB DDRB,2,temp	;out PB2 - PWM_F_R (2) - output (OC1B)
				OUTI TCCR1A,0<<COM1A0|1<<COM1A1|0<<COM1B0|1<<COM1B1|1<<WGM10|0<<WGM11	;Set register to Fast PWM 8-bit BOTTOM-TOP
				OUTI TCCR1B,1<<WGM12|0<<WGM13|0<<CS12|0<<CS11|1<<CS10					;Set register for Fast PWM 1:1
      			SETB DDRB,3,temp	;out PB3 - PWM_F_L (1) - output (OC2A)
			 	OUTI TCCR2A,0<<COM2A0|1<<COM2A1|0<<COM2B0|0<<COM2B1|1<<WGM20|1<<WGM21	;Set register to Fast PWM 8-bit BOTTOM-TOP
				OUTI TCCR0B,1<<WGM22|0<<CS22|0<<CS21|1<<CS20							;Set register for Fast PWM 1:1																						;need to set timers
...
STOP:			CLI						;Disable interrupts
				;STOP ALL MOTORS
				CLRB PORTD,0,temp		;MOTOR (4) LOW
      			CLRB PORTB,7,temp		;MOTOR (3) LOW
      			CLRB PORTB,4,temp		;MOTOR (2) LOW
      			CLRB PORTB,5,temp		;MOTOR (1) LOW
				OUTI OCR0B,0			;Stop REAR_RIGHT(2)
				OUTI OCR1AH,0			;Stop REAR_LEFT(1)(high and low bits)
				OUTI OCR1AL,0
				OUTI OCR1BH,0			;Stop FRONT_RIGHT (2)(high and low bits)
				OUTI OCR1BL,0
				OUTI OCR2A,0			;Stop FRONT_LEFT(1)
				;==========================================
				RJMP IDLE				;Make loop
;=====================================================================================================
INT_I2C:		CLI
...
;====================================================================================
FWARD:			SEI
				;Set directiom of rotation of MOTORS
      			SETB PORTD,0,temp			;MOTOR (4) HIGH
      			CLRB PORTB,7,temp			;MOTOR (3) LOW
      			SETB PORTB,4,temp			;MOTOR (2) HIGH
      			CLRB PORTB,5,temp			;MOTOR (1) LOW
				;Set PWM date from mainVar
				CP left_count,right_count	;Check value of encoders counts
				BRCS CORRECTION_L			;Goto correction if left side slower thet right
				CP right_count,left_count	;Check value of encoders counts
				BRCS CORRECTION_R			;Goto correction if right side slower thet left
				LD temp,Y+
				STS OCR0B,temp				;PWM for REAR_RIGHT(2)
				LD temp,Y+
				OUTI OCR1AH,0				;PWM for REAR_LEFT(1)(high and low bits)
				STS OCR1AL,temp
				LD temp,Y+
				OUTI OCR1BH,0				;PWM for FRONT_RIGHT(2)(high and low bits)
				STS OCR1BL,temp
				LD temp,Y+
				STS OCR2A,temp				;PWM FRONT_LEFT(1)
				RJMP IDLE					;Go to IDLE

CORRECTION_L:	LD temp,Y+
				OUTI OCR0B,0				;PWM for REAR_RIGHT(2)
				LD temp,Y+
				OUTI OCR1AH,0				;PWM for REAR_LEFT(1)(high and low bits)
				STS OCR1AL,temp
				LD temp,Y+
...
CORRECTION_R:	LD temp,Y+
				...
				STS OCR1BL,temp
				LD temp,Y+
				OUTI OCR2A,0				;PWM FRONT_LEFT(1)
				CP right_count, left_count	;Check value of encoders counts
				BRCS CORRECTION_R
				RJMP IDLE					;Go to IDLE

BACKWARD:		SEI
				;Set direction of rotation of MOTORS
      			CLRB PORTD,0,temp			;MOTOR (4) LOW
      			SETB PORTB,7,temp			;MOTOR (3) HIGH
      			CLRB PORTB,4,temp			;MOTOR (2) LOW
