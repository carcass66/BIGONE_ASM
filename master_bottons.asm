...
RESET:		LDI temp,Low(RAMEND)		;????????????? ?????
		...
		OUT PORTB,temp			
		NOP
		SETB PORTD,0,temp	;PULL-UP
		SETB PORTD,1,temp	;PULL-UP
		SETB PORTD,2,temp	;PULL-UP
		SETB PORTD,3,temp	;PULL-UP
		jmp MAIN			;????????? ? ??????? ????
;====================================================================================
INT_I2C:	CLI				;????????? ?????????? (?????????)
		LDS temp,TWSR			;????????? ???????? ??????-???????? ? ??????????? ????? ??? ?????? ????????????? ??? ????????
...
COLLIS:		RCALL TWI_Stop					
		ldi try,3
		RJMP Vix
SLA_R_ACK:	RCALL TWI_Stop					
		ldi try,3
		RJMP Vix
SLA_R_NACK:	RCALL TWI_Stop					
		ldi try,3
		RJMP Vix
REC_BYTE:	RCALL TWI_Stop			
		ldi try,3
		RJMP Vix
REC_BYTE_NACK:	RCALL TWI_Stop				
		ldi try,3
		RJMP Vix
Vix:		;SEI				;????????? ??????????(?????????)
		RETI				;??????? ?? ???????? ??????????
